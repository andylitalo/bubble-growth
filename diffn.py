...
def calc_dcdt_cyl(r_arr, c_arr, dr, dc, fixed_params):
    """
    ...
    dr : float
        mesh spacing [m]
    dc : float
        step size of concentration used to estimate dD/dc by forward difference
    ...
    polyol_data_file = fixed_params[0]
    ...
    D_arr = np.asarray([polyco2.calc_D(c, polyol_data_file) \
                                                    for c in list(c_arr_c)])
    # computes spatial derivative of concentration dc/dr with leapfrog method
    dcdr_arr = (c_arr[2:] - c_arr[:-2]) / (2*dr)
    ...
    # difference formula
    d2cdr2_arr = (c_arr[2:] - 2*c_arr[1:-1] + c_arr[:-2]) / (dc**2)
...
def go(dt, t_f, r_min, r_max, N, c_0, dcdt_fn, bc_specs_list, polyol_data_file):
    """
    ...
    t_f : float
        final time [s]
    r_min : float
        minimum radius of domain [m]
    r_max : float
        maximum radius of domain [m]
    N : int
        number of steps to make grid (N+1 grid points)
    ...
    t = [0]
    c = [c_0]
    r_arr = np.arange(0, N+1, 1)*dr

    # stores fixed parameters
    fixed_params = (polyol_data_file)
...
def neumann(c, i, j, dcdr, dr):
    ...
    dcdr : float
        ...
        [kg CO2 / m^3 polyol-CO2]
    dr : float
        grid spacing (used to compute derivative with difference formula) [m]
    """
...
def time_step(dt, t_prev, r_arr, c_prev, dcdt_fn, bc_specs_list, fixed_params):
    ...
    c = c_prev + dt*dcdt_fn(r_arr, np.array(c_prev), dr, dc, fixed_params)
